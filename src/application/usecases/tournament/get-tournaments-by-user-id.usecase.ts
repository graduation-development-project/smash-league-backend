...
@Injectable()
export class GetTournamentsByUserIdUseCase {
	constructor(
	...
	async execute(
		...
		return new ApiResponse<IPaginatedOutput<ITournamentResponse>>(
			HttpStatus.OK,
			"Get all tournaments by user id successful!",
			await this.tournamentRepository.getTournamentsByUserId(userId, options),
		);
	}
}
