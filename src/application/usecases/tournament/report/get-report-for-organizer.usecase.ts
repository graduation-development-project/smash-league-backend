...
@Injectable()
export class GetReportForOrganizerUseCase {
	constructor(
		@Inject("ReportRepository")
		private readonly reportRepository: ReportRepositoryPort,
	) {}

	async execute(organizerId: string): Promise<ApiResponse<UserReport[]>> {
		const reports =
			await this.reportRepository.getReportByOrganizerId(organizerId);
		if (reports.length === 0)
